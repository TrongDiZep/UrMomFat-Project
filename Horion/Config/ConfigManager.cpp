...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (Game.getLocalPlayer() != nullptr) {
			...
			Game.getGuiData()->displayClientMessageF("[%sMelody%s] %sSuccessfully %s config %s%s%s!", DARK_PURPLE, WHITE, GREEN, !configExists ? "created" : "loaded", GRAY, name.c_str(), GREEN);
